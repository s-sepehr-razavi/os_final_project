...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE || p->proc_round <= p->thread_round){
        if (p->pid > 2 && counter < 20)
        {          
          cprintf("******parent_id: %d, pid:%d, proc_round:%d, thread_round:%d\n",p->parent_process_id, p->pid, p->proc_round, p->thread_round);
          counter++;
        }
      ...
      struct proc *p_i;
      int min = p->proc_round; //minimum number of times that a thread should be executed
      int id = p->parent_process_id;
      int flag = 1;      
      for(p_i = ptable.proc; p_i < &ptable.proc[NPROC]; p_i++)
      {
        if (id == p_i->parent_process_id && min > p_i->thread_round) //BUG
        {
          flag = 0;
        }
      ...
      {
        struct proc *p_j;
        for(p_j = ptable.proc; p_j < &ptable.proc[NPROC]; p_j++)
        {
          if (id == p_j->parent_process_id) //BUG
          {
            p_j->proc_round++;                        
          }
...
void copy_process_state(struct proc *new_proc, struct proc *parent) {
    ...
    *new_proc->tf = *parent->tf;
    new_proc->parent_process_id = parent->parent_process_id;
    // cprintf("thread_id:%d, parent_id:%d\n", new_proc->pid, parent->pid);
    new_proc->proc_round = parent->proc_round;
    new_proc->thread_round = parent->proc_round;
    // cprintf("proc_round:%d, thread_round:%d\n", new_proc->proc_round, parent->thread_round);
}
